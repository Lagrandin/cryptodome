...
Cryptodome is a self-contained Python package of low-level
cryptographic primitives.

It supports Python 2.7, Python 3.5 and newer, and PyPy.

The installation procedure depends on the package you want the library to be in.
Cryptodome can be used as:

#. **an almost drop-in replacement for the old PyCrypto library**.
   You install it by cloning the repository and running::

       python setup.py

   In this case, all modules are installed under the ``Crypto`` package.

   One must avoid having both PyCrypto and Cryptodome installed
...
#. **a library independent of the old PyCrypto**.
   ...
   PyCrypto and Cryptodome can coexist.

For faster public key operations in Unix, you should install `GMP`_ in your system.

Cryptodome is a fork of PyCrypto. It brings the following enhancements
...
Cryptodome is not a wrapper to a separate C library like *OpenSSL*.
...
For security issues, please send an email to security@cryptodome.org.

All the code can be downloaded from `GitHub`_.

.. _`homepage`: https://www.cryptodome.org
.. _`GMP`: https://gmplib.org
.. _GitHub: https://github.com/Lagrandin/cryptodome
